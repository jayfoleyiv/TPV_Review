...
engines, they \textcolor{black}{are bounded by the Carnot limit and can
...
\textcolor{black}{efficiencies approaching 85\%}, only ST systems have yet been
...
\begin{center}
        ...
        \caption{\label{Ideal} \textcolor{black}{Maximum theoretical efficiency for various STPV system temperatures.  This ideal limiting
...
properties (the complex refractive index, \textcolor{black}{($\tilde{n} =
...
One such approach for defining such a profile involves using the \textcolor{black}{ultimate efficiency} of the emitter as a figure-of-merit,
where the \textcolor{black}{ultimate efficiency} depends on the emissivity of the structure, the band-gap of the PV cell, and the target 
...
This approach has been employed to design 1D photonic crystals involving tungsten and dielectric layers with \textcolor{black}{ultimate efficiencies} of 
...
This approach has predicted structures with \textcolor{black}{ultimate efficiencies} of 68\% at operating temperatures of 1750 K when coupled
...
as well as tungsten blazed grating emitter surfaces with \textcolor{black}{ultimate efficiencies} of 59\%~\cite{me2} (see Table 2).
...
to global optimization methods.  Generally, an appropriate figure-of-merit to be maximized is identified, e.g. the \textcolor{black}{ultimate efficiency} which will
...
\textcolor{black}{By transforming the incoming solar radiation from a
...
\begin{table}
	\caption{\textcolor{black}{Absorber efficiency corresponding to equation \ref{overall_sss_eq}, relative solar absorption, and relative thermal emission. 
...
\end{align}
where $\alpha(\lambda)$ is the spectral absorption of the surface, $\epsilon(\lambda)$ is the spectral emittance of the surface, \textcolor{black}{$C$ is the concentration factor of incoming sunlight, including concentrator ratio and concentrator efficiency,} $E_{sun}(\lambda)$ is the 
...
For emitting surfaces, a similar approach to absorbing surfaces can be taken, but with a focus on low reflectance in a narrow peak near a specific wavelength (which depends on the bandgap energy of the PV cell used), as opposed to a broad low reflectance band in the visible region.  To accurately compare emitting surfaces, their \textcolor{black}{ultimate efficiency} is used.

The \textcolor{black}{ultimate efficiency} is given by~\cite{me2}:
...
be evaluated on a case by case basis.  \textcolor{black}{We note that typically, the emissivity of the emitter structure
...
\begin{table}
	...
	\label{STPV_sys_table}
		...
		& coating &&&&&\\
		Graphite & {A}l$_2${O}$_3$/{E}r$_3${A}l$_5${O}$_{12}$ & GaSb & N/A & 25000 & 0.27 & 0.02 \cite{exp_tokyo} \\%(2000)\\
		& composite &&&&&\\
		\hline
